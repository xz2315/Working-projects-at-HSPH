...
proc tabulate data = predicted_mean   ;
  ...
  class   interv ;
  var meanY ;
  tables  interv, meanY*mean*f=12.8;
run;
...
proc tabulate data = predicted_mean   ;
  ...
  class dist interv ;
  var meanY ;
  tables dist, interv*meanY*mean*f=12.8;
run;
...
%baseline(y=quality1,ydist=Poisson,stratum=group1,out=2);
%baseline(y=preventive1,ydist=Poisson,stratum=group1,out=2);
%baseline(y=mgmt1,ydist=Poisson,stratum=group1,out=2);
...
%baseline(y=quality1,ydist=NEGBINOMIAL,stratum=group1,out=2 );
%baseline(y=preventive1,ydist=NEGBINOMIAL,stratum=group1,out=2 );
%baseline(y=mgmt1,ydist=NEGBINOMIAL,stratum=group1,out=2 );
...
proc glimmix data=data&y.  method=quad(Qpoints=100)   inititer=200  ;
	...
	*estimate "PGIP trend" penaltygroup 1 0 -1/ exp;
	...
	lsmeans &stratum.  /ilink  om at datapoint=1;
	lsmeans &stratum.  /ilink  om at datapoint=4;
...
proc tabulate data = predicted_mean   ;
    title "Parametric g-formula";
  class datapoint interv ;
  var meanY ;
  tables datapoint, interv*meanY*mean*f=12.8;
run;
...
proc glimmix data=data&y. method=quad(Qpoints=100)   inititer=200  ;
	...
	lsmeans dist*&stratum.  /ilink  om at datapoint=1;
	lsmeans dist*&stratum.  /ilink  om at datapoint=4;
	ods output ParameterEstimates=trendModel&y.;
	  output out = predicted_mean pred(noblup ilink)= meanY;
run;
data trendModel&y.;
...
proc tabulate data = predicted_mean   ;
    title "Parametric g-formula";
  class dist datapoint interv ;
  var meanY ;
  tables datapoint, dist*interv*meanY*mean*f=12.8;
run;
%end;
...
%trend(y=quality1,ydist=Poisson,stratum=group1,out=1);
%trend(y=preventive1,ydist=Poisson,stratum=group1,out=1);
%trend(y=mgmt1,ydist=Poisson,stratum=group1,out=1);

 * alternative distributional assumption;
* GAMMA default link is log;
* NB default link is log;
%trend(y=MedSurgCost,ydist=GAMMA,stratum=group1,out=1);
%trend(y=IPCost,ydist=GAMMA,stratum=group1,out=2);
%trend(y=OPCost,ydist=GAMMA,stratum=group1,out=1);
%trend(y=EDCost,ydist=GAMMA,stratum=group1,out=2);
%trend(y=DrugCost,ydist=GAMMA,stratum=group1,out=2);

%trend(y=ipdischarge,ydist=NEGBINOMIAL,stratum=group1,out=2);
%trend(y=edvisit,ydist=NEGBINOMIAL,stratum=group1,out=2);
%trend(y=readmission30,ydist=NEGBINOMIAL,stratum=group1,out=2);
%trend(y=readmission90,ydist=NEGBINOMIAL,stratum=group1,out=2);
%trend(y=pcpvisits,ydist=NEGBINOMIAL,stratum=group1,out=1);
%trend(y=specvisits,ydist=NEGBINOMIAL,stratum=group1,out=2);

%trend(y=quality1,ydist=NEGBINOMIAL,stratum=group1,out=1);
%trend(y=preventive1,ydist=NEGBINOMIAL,stratum=group1,out=1);
%trend(y=mgmt1,ydist=NEGBINOMIAL,stratum=group1,out=1);
