...
set apcd.DenominatorV21;
    sdate=MDY(BirthMonth,'01',BirthYear);
	if  Year=2011 then edate='31dec2011'd;
	else  if  Year=2012 then edate='31dec2012'd;
...
if age<18 then AgeGroup=1;
    ...
    if year=2011 then output bene2011;else if year=2012 then output bene2012; 
	keep MemberLinkEID  Year BirthMonth BirthYear Payer MedicaidIndicator MedicareCode Age AgeGroup Gender  
    State ZIP
    Plan1 Plan2 Plan3 Plan4 Type1 Type2 Type3 Type4  switcher;
run;
 
proc sql;
create table temp as
select *,case when MemberLinkEID in (select MemberLinkEID from bene2011 ) then 1 else 0 end as in2011
from bene2012 
where type1 ne '';
quit;
...
if Age <65  then step1=1;
if Age <65  and Switcher not in ("Medicaid, Medicare Advantage","Medicaid Managed Care, Medicare Advantage","Medicare Advantage, Private",
...
if Age <65  and Switcher not in ("Medicaid, Medicare Advantage","Medicaid Managed Care, Medicare Advantage","Medicare Advantage, Private",
"Medicaid, Medicaid Managed Care, Medicare Advantage","Medicaid, Medicare Advantage, Private","Medicaid Managed Care, Medicare Advantage, Private",
"Medicaid, Medicaid Managed Care, Medicare Advantage, Private","Only Medicare Advantage") and in2011=1 then step3=1;

if Age <65  and Switcher in ("Only Medicaid","Only Medicaid Managed Care","Only Private") and in2011=1 then step4=1;
proc freq;
table step1 step2 step3 step4;
run;
...
data denom;set denom;orgid=payer*1;
proc sort;by MemberLinkEID ORGID;run;
...
if Age <65 and Age >=18 then step1=1;

* Adult not MA;

if Age <65 and Age >=18 and Switcher not in ("Medicaid, Medicare Advantage","Medicaid Managed Care, Medicare Advantage","Medicare Advantage, Private",
"Medicaid, Medicaid Managed Care, Medicare Advantage","Medicaid, Medicare Advantage, Private","Medicaid Managed Care, Medicare Advantage, Private",
"Medicaid, Medicaid Managed Care, Medicare Advantage, Private","Only Medicare Advantage") then step2=1;
if Age <65 and Age >=18 and Switcher not in ("Medicaid, Medicare Advantage","Medicaid Managed Care, Medicare Advantage","Medicare Advantage, Private",
"Medicaid, Medicaid Managed Care, Medicare Advantage","Medicaid, Medicare Advantage, Private","Medicaid Managed Care, Medicare Advantage, Private",
"Medicaid, Medicaid Managed Care, Medicare Advantage, Private","Only Medicare Advantage") then step3=1;

* ADult with only one insurance;

if Age <65 and Age >=18 and Switcher in ("Only Medicaid","Only Medicaid Managed Care","Only Private")  then step4=1;
proc freq;
table step1 step2 step3 step4;
...
* get race;
proc sort data=apcd.DenominatorV21 out=temp3 nodupkey;where year=2012;by MemberLinkEID  ;run;
data temp3;
length race $30. ethnicity $30. hispanic $30.;
set temp3;

* recode Race/Ethnicity Hispanic;
if Race1 in ('R1') then Race="American Indian/Alaska Native";
else if Race1 in ('R2') then Race="Asian";
else if Race1 in ('R3') then Race="Black/African American";
else if Race1 in ('R4') then Race="Native Hawaiian or other Pacific Islander";
else if Race1 in ('R5') then Race="White";
else if Race1 in ('R9') then Race="Other Race";
else Race="Unkown/Missing";

if Ethnicity1 in ('AMERCN') then Ethnicity="American";
else if Ethnicity1 in ('BRAZIL') then Ethnicity="Brazilian";
else if Ethnicity1 in ('CVERDN') then Ethnicity="Cape Verdean";
else if Ethnicity1 in ('CARIBI') then Ethnicity="Caribbean Island";
else if Ethnicity1 in ('PORTUG') then Ethnicity="Portuguese";
else if Ethnicity1 in ('RUSSIA') then Ethnicity="Russian";
else if Ethnicity1 in ('EASTEU') then Ethnicity="Eastern European";
else if Ethnicity1 in ('OTHER') then Ethnicity="Other Ethnicity";
...
proc freq;tables seg2011 HighCost/missing;
run;
...
Link PQI Spending;
**************************;
proc sort data=APCD.AnalyticDataCost out=AnalyticDataCost;by MemberLinkEID;run;
data APCD.AnalyticDataCost ;
merge AnalyticDataCost(in=in1) APCD.Tapq01 APCD.Tapq02 APCD.Tapq03 APCD.Tapq05 APCD.Tapq07 
...
proc sort data=APCD.AnalyticDataSpending out=AnalyticDataSpending;by MemberLinkEID;run;
data APCD.AnalyticDataSpending;
merge AnalyticDataSpending(in=in1) APCD.Tapq01 APCD.Tapq02 APCD.Tapq03 APCD.Tapq05 APCD.Tapq07 
APCD.Tapq08 APCD.Tapq10 APCD.Tapq11 APCD.Tapq12 APCD.Tapq13 APCD.Tapq14 APCD.Tapq15 APCD.Tapq16
APCD.Tapq90 APCD.Tapq91 APCD.Tapq92;
by MemberLinkEID;
if in1=1;
