...
* each year, define 4 groups: FFS+FFS, FFS+MC, MC+FFS, MC+MC;
/*
E_MEDICAREFFS	Annual Medicare FFS Indicator	
1 - All months alive Medicare FFS
0 - Some months Medicare MA
Null - No months MA, not all months alive Medicare FFS

E_MEDICAIDFFS	Annual Medicaid FFS Indicator	
1 - All months alive Medicaid FFS
0 - Some months Medicaid PHP
Null - No months PHP, not all months alive Medicaid FFS

E_MEDICAREMA	Annual Medicare Managed Care Summary	
1 - Some eligible months MA
0 - FFS = No eligible months MA
Null - Not FFS or MA

E_MEDICAIDPHP	Annual Medicaid Managed Care (Prepaid Health Plan - PHP) Indicator	
4 - Some eligible months LTC
3 - No LTC but some eligible months CMC
2 - No LTC or CMC but some eligible months PCCM
1 - No LTC or CMC or PCCM but some eligible months LMC
0 - FFS = No LTC or CMC or PCCM or LMC
Null - Not FFS or MA
*/
data temp3&yr.;
length group1 $50.;
...
if E_MEDICAREFFS =1 and E_MEDICAIDFFS =1 then group1="Medicare FFS+Medicaid FFS";
else if E_MEDICAREFFS =1 and E_MEDICAIDFFS =0 then group1="Medicare FFS+Medicaid Managed Care";
else if E_MEDICAREFFS =0 and E_MEDICAIDFFS =1 then group1="Medicare Managed Care+Medicaid FFS";
else if E_MEDICAREFFS =0 and E_MEDICAIDFFS =0 then group1="Medicare Managed Care+Medicaid Managed Care";
if group1 ne'';
proc freq;tables group1/missing;
run;
...
run;
proc sql;
create table data.BeneV3&yr. as
select a.*,b.*
...
select  a.bene_id, a.S_PMT as S_PMT2008, a.S_Medicare_PMT as S_Medicare_PMT2008, a.S_Medicaid_PMT as S_Medicaid_PMT2008,a.group1 as group12008,
b.S_PMT as S_PMT2009,b.S_Medicare_PMT as S_Medicare_PMT2009, b.S_Medicaid_PMT as S_Medicaid_PMT2009,b.group1 as group12009
from data.beneV32008 a inner join data.beneV32009 b
on a.bene_id=b.bene_id;
quit;
proc sql;
create table bene2 as
select  a.*,  
b.S_PMT as S_PMT2010,b.S_Medicare_PMT as S_Medicare_PMT2010, b.S_Medicaid_PMT as S_Medicaid_PMT2010,b.group1 as group12010
from bene1 a inner join data.beneV32010 b
on a.bene_id=b.bene_id;
quit;

*Please check group1 change over year first, and let Jose decide how to proceed: only those didn't change;
data bene3;
set bene2;
if group12008=group12009 and group12009=group12010 ;
group1=group12010;
run;

proc rank data=bene3 out=bene4 percent;
var S_PMT2008 S_PMT2009 S_PMT2010 ;
ranks pct_PMT2008 pct_PMT2009 pct_PMT2010 ;
run;

data data.PlanABeneV3;
set bene4;
length group $30.;
...
proc freq;tables HC2008 HC2009 HC2010 group group1*group/missing nopercent norow nocol;
run;

* Look at cut point;
proc means data=data.PlanABeneV3 min mean median max;
class  group1;
var S_PMT2008 S_Medicare_PMT2008  S_Medicaid_PMT2008  ;
run;

proc means data=data.PlanABeneV3 min mean median max;
class group1;
var S_PMT2009 S_Medicare_PMT2009  S_Medicaid_PMT2009;
run;

proc means data=data.PlanABeneV3 min mean median max;
class group1;
var S_PMT2010 S_Medicare_PMT2010 S_Medicaid_PMT2010;
run;
