...
* County / State ;
proc import datafile="D:\Projects\APCD High Cost\ZIP_COUNTY_092015" dbms=xlsx out=ZIP  replace;getnames=yes;run;
proc import datafile='D:\Projects\APCD High Cost\county_to_zip_census2010.xlsx' out=County dbms=xlsx replace;getnames=yes;run;
...
libname ARF 'D:\Projects\Medicare Utilization vs MA Rate';
data temp1;
set temp1;
...
******************************************************************************;
libname betos 'D:\Projects\APCD High Cost\Archieve';
data temp;
set  
...
if type="Other Part B" then do;
	if betos="O1A" then type="AMBULANCE";
	else if betos="O1B" then type="CHIROPRACTIC";
	else if betos="O1C" then type="ENTERAL AND PARENTERAL";
	else if betos="O1D" then type="CHEMOTHERAPY";
	else if betos="O1E" then type="OTHER DRUGS";
	else if betos="O1F" then type="VISION, HEARING AND SPEECH SERVICES";
	else if betos="O1G" then type="INFLUENZA IMMUNIZATION";
	else  type="EXCEPTIONS/UNCLASSIFIED";
end;
...
%carrier(name=AMBULANCE,type=AMBULANCE,str="AMBULANCE");
%carrier(name=CHIROPRACTIC,type=CHIROPRACTIC,str="CHIROPRACTIC");
%carrier(name=ENTERAL,type=ENTERAL AND PARENTERAL,str="ENTERAL AND PARENTERAL");
%carrier(name=CHEMOTHERAPY,type=CHEMOTHERAPY,str="CHEMOTHERAPY");
%carrier(name=DRUGS,type=OTHER DRUGS,str="OTHER DRUGS");
%carrier(name=VISION,type=VISION HEARING AND SPEECH SERVICES,str="VISION, HEARING AND SPEECH SERVICES");
%carrier(name=INFLUENZA,type=AMBULANCE,str="INFLUENZA IMMUNIZATION");
%carrier(name=EXCEPTIONS,type=AMBULANCE,str="EXCEPTIONS/UNCLASSIFIED");
...
tempASC tempPTB_DRUG tempEM tempPROC tempIMG tempTESTS tempDME 
tempAMBULANCE tempCHIROPRACTIC tempENTERAL tempCHEMOTHERAPY tempDRUGS tempVISION tempINFLUENZA tempEXCEPTIONS  ;
by MemberLinkEID ORGID;
if in1=1;

array temp1 {28} Cost_IP Cost_IP_OTH Cost_PAC_OTH Cost_HH Cost_Hospice Cost_SNF
Cost_OPD Cost_ESRD Cost_FQHC Cost_RHC Cost_THRPY Cost_CMHC Cost_OTH40
Cost_ASC Cost_PTB_DRUG Cost_EM Cost_PROC Cost_IMG Cost_TESTS Cost_DME
Cost_AMBULANCE Cost_CHIROPRACTIC Cost_ENTERAL Cost_CHEMOTHERAPY Cost_DRUGS Cost_VISION Cost_INFLUENZA Cost_EXCEPTIONS;


array temp2 {28} Spending_IP Spending_IP_OTH Spending_PAC_OTH Spending_HH Spending_Hospice Spending_SNF
Spending_OPD Spending_ESRD Spending_FQHC Spending_RHC Spending_THRPY Spending_CMHC Spending_OTH40
Spending_ASC Spending_PTB_DRUG Spending_EM Spending_PROC Spending_IMG Spending_TESTS Spending_DME
Spending_AMBULANCE Spending_CHIROPRACTIC Spending_ENTERAL Spending_CHEMOTHERAPY Spending_DRUGS Spending_VISION Spending_INFLUENZA Spending_EXCEPTIONS;

array temp3 {28} CLM_IP CLM_IP_OTH CLM_PAC_OTH CLM_HH CLM_Hospice CLM_SNF
CLM_OPD CLM_ESRD CLM_FQHC CLM_RHC CLM_THRPY CLM_CMHC CLM_OTH40
CLM_ASC CLM_PTB_DRUG CLM_EM CLM_PROC CLM_IMG CLM_TESTS CLM_DME
CLM_AMBULANCE CLM_CHIROPRACTIC CLM_ENTERAL CLM_CHEMOTHERAPY CLM_DRUGS CLM_VISION CLM_INFLUENZA CLM_EXCEPTIONS;
...
Cost_OPD+ Cost_ESRD+ Cost_FQHC+ Cost_RHC+ Cost_THRPY+ Cost_CMHC+ Cost_OTH40+
Cost_ASC+ Cost_PTB_DRUG+ Cost_EM+ Cost_PROC+ Cost_IMG+ Cost_TESTS+  Cost_DME+
Cost_AMBULANCE+ Cost_CHIROPRACTIC+Cost_ENTERAL+ Cost_CHEMOTHERAPY+ Cost_DRUGS+ 
Cost_VISION+ Cost_INFLUENZA+ Cost_EXCEPTIONS;

Spending=Spending_IP+ Spending_IP_OTH+ Spending_PAC_OTH+ Spending_HH+ Spending_Hospice+ Spending_SNF+
Spending_OPD+ Spending_ESRD+ Spending_FQHC+ Spending_RHC+ Spending_THRPY+ Spending_CMHC+ Spending_OTH40+
Spending_ASC+ Spending_PTB_DRUG+ Spending_EM+ Spending_PROC+ Spending_IMG+ Spending_TESTS+ Spending_DME+
Spending_AMBULANCE+Spending_CHIROPRACTIC+ Spending_ENTERAL+ Spending_CHEMOTHERAPY+ Spending_DRUGS+ 
Spending_VISION+ Spending_INFLUENZA+ Spending_EXCEPTIONS;
...
PTBNI_Cost=Cost_ASC+ Cost_PTB_DRUG+ Cost_EM+ Cost_PROC+ Cost_IMG+ Cost_TESTS+ Cost_DME+
Cost_AMBULANCE+ Cost_CHIROPRACTIC+Cost_ENTERAL+ Cost_CHEMOTHERAPY+ Cost_DRUGS+ 
Cost_VISION+ Cost_INFLUENZA+ Cost_EXCEPTIONS;
...
PTBNI_Spending=Spending_ASC+ Spending_PTB_DRUG+ Spending_EM+ Spending_PROC+ Spending_IMG+ Spending_TESTS+ Spending_DME+
Spending_AMBULANCE+Spending_CHIROPRACTIC+ Spending_ENTERAL+ Spending_CHEMOTHERAPY+ Spending_DRUGS+ 
Spending_VISION+ Spending_INFLUENZA+ Spending_EXCEPTIONS;
...
sum(Cost_ASC) as T_Cost_ASC ,sum(Cost_PTB_DRUG) as T_Cost_PTB_DRUG ,sum(Cost_EM) as T_Cost_EM ,sum(Cost_PROC) as T_Cost_PROC ,sum(Cost_IMG) as T_Cost_IMG ,sum(Cost_TESTS) as T_Cost_TESTS ,sum(Cost_DME) as T_Cost_DME ,
sum(Cost_AMBULANCE) as T_Cost_AMBULANCE ,sum(Cost_CHIROPRACTIC) as T_Cost_CHIROPRACTIC ,sum(Cost_ENTERAL) as T_Cost_ENTERAL ,
sum(Cost_CHEMOTHERAPY) as T_Cost_CHEMOTHERAPY ,sum(Cost_DRUGS) as T_Cost_DRUGS ,sum(Cost_VISION) as T_Cost_VISION ,
sum(Cost_INFLUENZA) as T_Cost_INFLUENZA ,sum(Cost_EXCEPTIONS) as T_Cost_EXCEPTIONS ,
...
sum(Spending_ASC) as T_Spending_ASC ,sum(Spending_PTB_DRUG) as T_Spending_PTB_DRUG ,sum(Spending_EM) as T_Spending_EM ,sum(Spending_PROC) as T_Spending_PROC ,sum(Spending_IMG) as T_Spending_IMG ,sum(Spending_TESTS) as T_Spending_TESTS ,sum(Spending_DME) as T_Spending_DME ,
sum(Spending_AMBULANCE) as T_Spending_AMBULANCE ,sum(Spending_CHIROPRACTIC) as T_Spending_CHIROPRACTIC ,sum(Spending_ENTERAL) as T_Spending_ENTERAL ,
sum(Spending_CHEMOTHERAPY) as T_Spending_CHEMOTHERAPY ,sum(Spending_DRUGS) as T_Spending_DRUGS ,sum(Spending_VISION) as T_Spending_VISION ,
sum(Spending_INFLUENZA) as T_Spending_INFLUENZA ,sum(Spending_EXCEPTIONS) as T_Spending_EXCEPTIONS ,
...
sum(CLM_ASC) as T_CLM_ASC ,sum(CLM_PTB_DRUG) as T_CLM_PTB_DRUG ,sum(CLM_EM) as T_CLM_EM ,sum(CLM_PROC) as T_CLM_PROC ,sum(CLM_IMG) as T_CLM_IMG ,sum(CLM_TESTS) as T_CLM_TESTS ,sum(CLM_DME) as T_CLM_DME ,
sum(CLM_AMBULANCE) as T_CLM_AMBULANCE ,sum(CLM_CHIROPRACTIC) as T_CLM_CHIROPRACTIC ,sum(CLM_ENTERAL) as T_CLM_ENTERAL ,
sum(CLM_CHEMOTHERAPY) as T_CLM_CHEMOTHERAPY ,sum(CLM_DRUGS) as T_CLM_DRUGS ,sum(CLM_VISION) as T_CLM_VISION ,
sum(CLM_INFLUENZA) as T_CLM_INFLUENZA ,sum(CLM_EXCEPTIONS) as T_CLM_EXCEPTIONS ,
...
label T_Cost_IP ="Standard Cost: Inpatient Acute Care Hospital (include CAH) ";
label T_Cost_IP_OTH ="Standard Cost: Other Inpatient (Psych/other hospital)";
label T_Cost_PAC_OTH ="Standard Cost: Other post-acute care (Inpatient Rehab, long-term hospital)";
label T_Cost_HH ="Standard Cost: Home Health (HH)";
label T_Cost_Hospice ="Standard Cost: Hospice";
label T_Cost_SNF ="Standard Cost: Skilled Nursing Facility (SNF)";
label T_Cost_OPD ="Standard Cost: Outpatient Department Services";
label T_Cost_ESRD ="Standard Cost: End Stage Renal Disease (ESRD) Facility";
label T_Cost_FQHC ="Standard Cost: Federally Qualified Health Center (FQHC)";
label T_Cost_RHC ="Standard Cost: Rural Health Center (Clinic)";
label T_Cost_THRPY ="Standard Cost: Outpatient Therapy (outpatient rehab and community outpatient rehab)";
label T_Cost_CMHC ="Standard Cost: Community Mental Health Center (CMHC)";
label T_Cost_OTH40 ="Standard Cost: Other";
label T_Cost_ASC ="Standard Cost: Ambulatory Surgical Center (ASC)";
label T_Cost_PTB_DRUG ="Standard Cost: Part B Drug";
label T_Cost_EM ="Standard Cost: Physician Evaluation and Management";
label T_Cost_PROC ="Standard Cost: Procedures";
label T_Cost_IMG ="Standard Cost: Imaging";
label T_Cost_TESTS ="Standard Cost: Tests";
label T_Cost_DME ="Standard Cost: Durable Medical Equipment (DME)";
label T_Cost_AMBULANCE ="Standard Cost: AMBULANCE";
label T_Cost_CHIROPRACTIC ="Standard Cost: CHIROPRACTIC";
label T_Cost_ENTERAL="Standard Cost: ENTERAL AND PARENTERAL";
label T_Cost_CHEMOTHERAPY ="Standard Cost: CHEMOTHERAPY";
label T_Cost_DRUGS="Standard Cost: OTHER DRUGS"; 
label T_Cost_VISION ="Standard Cost: VISION, HEARING AND SPEECH SERVICES";
label T_Cost_INFLUENZA ="Standard Cost: INFLUENZA IMMUNIZATION";
label T_Cost_EXCEPTIONS ="Standard Cost: EXCEPTIONS/UNCLASSIFIED";
        

label T_Spending_IP ="Spending: Inpatient Acute Care Hospital (include CAH)";
label T_Spending_IP_OTH ="Spending: Other Inpatient (Psych/other hospital)";
label T_Spending_PAC_OTH="Spending: Other post-acute care (Inpatient Rehab, long-term hospital)";
label T_Spending_HH ="Spending: Home Health (HH)";
label T_Spending_Hospice ="Spending: Hospice";
label T_Spending_SNF ="Spending: Skilled Nursing Facility (SNF)";
label T_Spending_OPD ="Spending: Outpatient Department Services";
label T_Spending_ESRD ="Spending: End Stage Renal Disease (ESRD) Facility";
label T_Spending_FQHC ="Spending: Federally Qualified Health Center (FQHC)";
label T_Spending_RHC ="Spending: Rural Health Center (Clinic)";
label T_Spending_THRPY ="Spending: Outpatient Therapy (outpatient rehab and community outpatient rehab)";
label T_Spending_CMHC ="Spending: Community Mental Health Center (CMHC)";
label T_Spending_OTH40 ="Spending: Other";
label T_Spending_ASC ="Spending: Ambulatory Surgical Center (ASC)";
label T_Spending_PTB_DRUG ="Spending: Part B Drug";
label T_Spending_EM ="Spending: Physician Evaluation and Management";
label T_Spending_PROC ="Spending: Procedures";
label T_Spending_IMG ="Spending: Imaging";
label T_Spending_TESTS ="Spending: Tests";
label T_Spending_DME ="Spending: Durable Medical Equipment (DME)";
label T_Spending_AMBULANCE="Spending: AMBULANCE"; 
label T_Spending_CHIROPRACTIC ="Spending: CHIROPRACTIC";
label T_Spending_ENTERAL ="Spending: ENTERAL AND PARENTERAL";
label T_Spending_CHEMOTHERAPY ="Spending: CHEMOTHERAPY";
label T_Spending_DRUGS ="Spending: OTHER DRUGS";
label T_Spending_VISION ="Spending: VISION, HEARING AND SPEECH SERVICES";
label T_Spending_INFLUENZA ="Spending: INFLUENZA IMMUNIZATION";
label T_Spending_EXCEPTIONS ="Spending: EXCEPTIONS/UNCLASSIFIED";

label T_CLM_IP ="N.of Claims: Inpatient Acute Care Hospital (include CAH) ";
label T_CLM_IP_OTH ="N.of Claims: Other Inpatient (Psych/other hospital)";
label T_CLM_PAC_OTH ="N.of Claims: Other post-acute care (Inpatient Rehab, long-term hospital)";
label T_CLM_HH="N.of Claims: Home Health (HH)";
label T_CLM_Hospice="N.of Claims: Hospice";
label T_CLM_SNF ="N.of Claims: Skilled Nursing Facility (SNF)";
label T_CLM_OPD ="N.of Claims: Outpatient Department Services";
label T_CLM_ESRD ="N.of Claims: End Stage Renal Disease (ESRD) Facility";
label T_CLM_FQHC ="N.of Claims: Federally Qualified Health Center (FQHC)";
label T_CLM_RHC ="N.of Claims: Rural Health Center (Clinic)";
label T_CLM_THRPY ="N.of Claims: Outpatient Therapy (outpatient rehab and community outpatient rehab)";
label T_CLM_CMHC ="N.of Claims: Community Mental Health Center (CMHC)";
label T_CLM_OTH40 ="N.of Claims: Other";
label T_CLM_ASC ="N.of Claims: Ambulatory Surgical Center (ASC)";
label T_CLM_PTB_DRUG ="N.of Claims: Part B Drug";
label T_CLM_EM="N.of Claims: Physician Evaluation and Management";
label T_CLM_PROC ="N.of Claims: Procedures";
label T_CLM_IMG ="N.of Claims: Imaging";
label T_CLM_TESTS="N.of Claims: Tests";
label T_CLM_DME ="N.of Claims: Durable Medical Equipment (DME)";
label T_CLM_AMBULANCE="N.of Claims: AMBULANCE";
label T_CLM_CHIROPRACTIC ="N.of Claims: CHIROPRACTIC";
label T_CLM_ENTERAL ="N.of Claims: ENTERAL AND PARENTERAL";
label T_CLM_CHEMOTHERAPY ="N.of Claims: CHEMOTHERAPY";
label T_CLM_DRUGS="N.of Claims: OTHER DRUGS";
label T_CLM_VISION ="N.of Claims: VISION, HEARING AND SPEECH SERVICES";
label T_CLM_INFLUENZA ="N.of Claims: INFLUENZA IMMUNIZATION";
label T_CLM_EXCEPTIONS ="N.of Claims: EXCEPTIONS/UNCLASSIFIED";

label T_LOS_IP ="LOS: Inpatient Acute Care Hospital (include CAH) ";
label T_LOS_IP_OTH ="LOS: Other Inpatient (Psych/other hospital)";
label T_LOS_PAC_OTH ="LOS: Other post-acute care (Inpatient Rehab, long-term hospital)";
label T_LOS_HH ="LOS: Home Health (HH)";
label T_LOS_Hospice ="LOS: Hospice";
label T_LOS_SNF ="LOS: Skilled Nursing Facility (SNF)";

label T_PTA_Cost="Part A Cost";
label T_PTBI_Cost="Part B Institutional Cost";
label T_PTBNI_Cost="Part B Non-Institutional Cost";
label T_PTA_Spending="Part A Spending";
label T_PTBI_Spending="Part B Institutional Spending";
label T_PTBNI_Spending="Part B Non-Institutional Spending";

label T_Cost="Total Cost";
label T_Spending="Total Spending";
